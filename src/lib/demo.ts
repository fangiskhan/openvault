...
// The illustrative demo dataset: three projects (Atlas <-> Orion connected, Nova
// isolated), cross-linked notes, a blocked overdue task and an open risk — so the
// Status board, Graph, and MCP tools all have something real to show on a fresh
// vault. Additive by design (it only creates rows); the CLI seed clears the
// tables first for an idempotent reseed, the /api/demo route guards on emptiness.
export async function seedDemoData() {
  ...
  return { projects: [atlas, orion, nova], noteCount: all.length };
}
